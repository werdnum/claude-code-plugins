"""TypeScript/JavaScript linting support with prettier and eslint."""
...
async def run_prettier(file_path: str, config: dict[str, Any]) -> LintResult:
    ...
    project_dir = config.get("projectDir", "frontend")
    file_path_obj = Path(file_path)
    frontend_dir = Path.cwd() / project_dir

    # Check if file is in the frontend directory
    try:
        file_path_obj.relative_to(frontend_dir)
    except ValueError:
        # File not in frontend directory
        return LintResult("prettier", True, 0.0)
...
async def lint_file(file_path: str, tool_name: str, tool_input: dict[str, Any], config: dict[str, Any]) -> dict[str, Any] | None:
    """Run TypeScript/JavaScript linters on a file."""
    tasks = [
