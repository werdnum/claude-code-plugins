...
2. **Use Appropriate Research Tools**:
   ...
   - **DeepWiki (preferred for open-source repos)**: Use the DeepWiki MCP tools to research public
     GitHub repositories. DeepWiki provides AI-generated documentation and search for open-source
     projects, making it the fastest way to understand a library's architecture, API, and internals:
     - `mcp__deepwiki__read_wiki_structure` to get the documentation topics for a repo
     - `mcp__deepwiki__read_wiki_contents` to read specific documentation pages
     - `mcp__deepwiki__ask_question` to ask AI-powered questions about a repo's codebase
     - Use the format `owner/repo` (e.g. `facebook/react`, `pallets/flask`) when specifying repos
...
- For open-source library/framework research, start with DeepWiki (`mcp__deepwiki__ask_question`)
  to quickly understand architecture, APIs, and internals before turning to other sources
- Use targeted web searches for official documentation and non-GitHub sources
